...
function createQuickChatManager(deps = {}) {
  const {
    ...
    if (!rendererApiGlobal) {
        try {
            console.warn(
                '[quick-chat] createQuickChatManager: rendererApiGlobal not provided; '
                + "renderer-agent calls will fall back to the shared default '__appRenderer'. "
                + 'Pass rendererApiGlobal from app.config.js via main.js initQuickChat().'
            );
        } catch {}
    }

    // Same rendererApiOptions pattern used by createExporters(...) and
    // createLayoutCSS(...): pin every renderer-agent call to the app's
    // configured global so it never falls back to '__appRenderer'.
...
function promptForText(parentWin, { title = 'Rename', message = 'Name:', defaultValue = '' } = {}) {
    return new Promise(resolve => {
        ...
        function done(value) {
            if (resolved) return;
            resolved = true;
            try { ipcMain.removeListener('quick-chat-rename-result', onResult); } catch {}
            try {
        ...
        function onResult(event, payload) {
        ...
        try {
            const parent = parentWin || getMain() || undefined;
            ...
            promptWin = new BrowserWindow({
                ...
                webPreferences: {
                    ...
                    sandbox: true,
                    preload: path.join(dirname, 'renderer', 'prompt-preload.js'),
                },
            });

            promptWin.removeMenu();

            ipcMain.on('quick-chat-rename-result', onResult);

            promptWin.once('ready-to-show', () => {
                try { promptWin.show(); promptWin.focus(); } catch {}
                try {
                    promptWin.webContents.send('prompt:init', {
                        title: String(title || 'Rename'),
            ...
            promptWin.webContents.on('did-fail-load', (_e, code, desc, url) => {
                try { console.error('promptForText loadFile failed:', code, desc, url); } catch {}
                done(null);
            });

            promptWin.loadFile(
                path.join(dirname, 'renderer', 'prompt.html')
            ).catch((err) => {
                try { console.error('promptForText loadFile threw:', err); } catch {}
                done(null);
            });
        } catch (err) {
            try { console.error('promptForText setup failed:', err); } catch {}
            done(null);
        }
    });
}
  ...
  async function renameQuickChatWindow(win) {
    ...
    if (!value || typeof value.name !== 'string') return;
      win.__quickName = value.name.trim();
      updateQuickWindowTitle(win);
      refreshQuickChatMenu();
    } catch (err) {
  ...
  function delayMs(ms) {
    ...
    async function waitForChatInput(wc, timeoutMs = 4000) {
        ...
        if (result.missing) {
            try {
                console.warn(
                    '[quick-chat] waitForChatInputReady MISSING on renderer agent:',
                    result
                );
            } catch {}
            return false;
        ...
        if (!result.ok) {
            try {
                console.warn('[quick-chat] waitForChatInputReady failed:', result);
            } catch {}
            return false;
        }
        ...
        if (result.timeout) {
            try {
                console.warn('[quick-chat] chat input did not become ready before timeout');
            } catch {}
            return false;
        }
