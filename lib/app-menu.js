...
// --- View menu: Expand submenu --------------------------------------------
// Expanding the conversation is a deliberate USER action here, not something
// the export tries to do on the fly. See expandChatPane() in lib/exporters.js
// for the measured reasons expansion cannot work inside the export pipeline.
// Once expanded, both "Export Chat Pane" and Find operate on a fully mounted,
// fully rendered, content-visible DOM.
function appendViewItems(viewSubmenu) {
  // Guard against overlapping runs: the expansion can take a minute or more,
  // and two concurrent walks would fight over scroll position.
  let expandInProgress = false;

  async function runExpand(includeReasoning, label) {
    const w = BrowserWindow.getFocusedWindow() || getMainWindow();
    if (!w) return;
    if (expandInProgress) {
      try {
        dialog.showMessageBox(w, {
          type: 'info',
          message: 'An expansion is already running.',
          detail: 'Press Escape in the chat window to cancel it.',
          buttons: ['OK'],
        });
      } catch {}
      return;
    ...
    try {
      const res = await expandChatPane(w, { includeReasoning });
      if (!res?.ok) {
        const why = res?.reason === 'chat-pane-not-found'
          ? 'Could not find the chat pane.'
          : String(res?.reason || 'Unknown error');
        try { dialog.showErrorBox(label, why); } catch {}
        return;
      }
      const r = res.reasoning;
      if (includeReasoning && r) {
        console.log('[view-expand] reasoning panels:', {
          found: r.found, clicked: r.clicked,
          captured: r.capturedPanels, stillThin: r.materializeStillThin,
          cancelled: r.cancelled
        });
      }
      if (res.cancelled) {
        console.log('[view-expand] cancelled by user (Escape)');
        try {
          dialog.showMessageBox(w, {
            type: 'info',
            message: 'Expansion cancelled.',
            detail: 'Whatever was already expanded has been kept.',
            buttons: ['OK'],
          });
        } catch {}
      }
      ...
      try { dialog.showErrorBox(label + ' failed', String(err?.message || err)); } catch {}
    } finally {
  ...
  const template = [
    { type: 'separator' },
    {
      ...
      submenu: [
        {
          id: 'app-view-expand-all',
          label: 'Expand All (including reasoning)',
          click: () => runExpand(true, 'Expand All'),
        },
        {
          id: 'app-view-expand-no-reasoning',
          label: 'Expand All (except reasoning)',
          click: () => runExpand(false, 'Expand All (except reasoning)'),
        },
      ],
    },
  ];
  Menu.buildFromTemplate(template).items.forEach(i => viewSubmenu.append(i));
}
...
function augmentApplicationMenu(win) {
  ...
  // Ensure "View" submenu exists, then append the Expand items. Electron's
  // default application menu already provides View (reload, zoom, devtools,
  // full screen); this only appends to it. The fallback insert covers the case
  // where the default menu is unavailable.
  let viewSubmenu = appMenu.items.find(i => i.label === 'View')?.submenu;
  if (!viewSubmenu) {
    viewSubmenu = new Menu();
    const viewItem = new MenuItem({ label: 'View', submenu: viewSubmenu });
    const editIndex = appMenu.items.findIndex(i => i.label === 'Edit');
    if (editIndex >= 0) appMenu.insert(editIndex + 1, viewItem);
    else appMenu.append(viewItem);
  }
  if (!submenuHasManagedItem(viewSubmenu, { id: 'app-view-expand', label: 'Expand' })) {
    appendViewItems(viewSubmenu);
  }
...
// ---------- end File menu ----------
  ...
  return {
    appendEditItems, appendHelpItems, appendSessionItems, appendViewItems,
